...
void main() async {
  ...
  try {
    if (dbType == 'turso') {
      ...
      await client.initTursoDatabase(databaseUrl, authToken);
    } else {
      final dbPath = env['DATABASE_PATH'] ?? 'example.db';
      print('Initializing local database: $dbPath');
      await client.initLocalDatabase(dbPath);
    }
    ...
    for (final med in medications) {
      ...
      if (med.reminders != null && med.reminders!.isNotEmpty) {
    ...
    print('\n--- Syncing Database (Turso only) ---\n');

    // Sync database if using Turso
    if (dbType == 'turso') {
      try {
        await client.syncDatabase();
        print('Database synced successfully');
      } catch (e) {
        print('Sync not needed for local database or error: $e');
      }
    } else {
      print('Sync not needed for local database');
    }
